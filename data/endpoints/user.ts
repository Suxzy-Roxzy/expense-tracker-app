import { UserType } from "@/validators/types/user.js";
import {CreateUserSchemaType, UsersSchema, UserSchema, UsersSchemaType, UpdateUserSchemaType} from "../../validators/schemas/user.js";
  ...
  return useMutation({
    mutationFn: async({
      ...
      const response = await AxiosInstance.put(
        `/api/v1/user/${user_id}`, updatedUser
      );
      return response.data;
    },
    onSuccess: () => {
  ...
  return useMutation({
    mutationFn: async({user_id, newRole}: changeRoleType) => {
      const response = await AxiosInstance.post(
        `/api/v1/user/change-role`, {user_id, newRole}
      );
